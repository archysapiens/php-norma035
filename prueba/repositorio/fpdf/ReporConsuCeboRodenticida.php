...
require_once 'fpdi.php';
	...
	$fileJson = '../../repositorio/DataJson/DataFumigacion/ReportRodenticidas/'.$sucursal.'/dataReport'.$noCliente.'.json';
	$str_datos = json_decode(file_get_contents("$fileJson"),true);

$pdf = new FPDI();
$pageCount = $pdf->setSourceFile("ReporteRodenticidaActual.pdf");
...
{	
	...
	// @$limpieza=utf8_decode(@$ExplodeCamposGrid[$i][3]); @$observacion=utf8_decode(@$ExplodeCamposGrid[$i][5]);
	// if(utf8_decode(@$ExplodeCamposGrid[$i][2])==0){$CampoConsumo4= $CampoConsumo2= $CampoConsumo3= $CampoConsumo5 = ""; $CampoConsumo1 = utf8_decode('x');
	// }elseif(utf8_decode(@$ExplodeCamposGrid[$i][2])==25){$CampoConsumo4= $CampoConsumo1= $CampoConsumo3= $CampoConsumo5 = ""; $CampoConsumo2 = utf8_decode('x');
	// }elseif(utf8_decode(@$ExplodeCamposGrid[$i][2])==50){$CampoConsumo4= $CampoConsumo2= $CampoConsumo1= $CampoConsumo5 = ""; $CampoConsumo3 = utf8_decode('x');
	// }elseif(utf8_decode(@$ExplodeCamposGrid[$i][2])==75){$CampoConsumo1= $CampoConsumo2= $CampoConsumo3= $CampoConsumo5 = ""; $CampoConsumo4 = utf8_decode('x');
	// }elseif(utf8_decode(@$ExplodeCamposGrid[$i][2])==100){$CampoConsumo4= $CampoConsumo2= $CampoConsumo3= $CampoConsumo1 = ""; $CampoConsumo5 = utf8_decode('x'); }

	// if(utf8_decode(@$ExplodeCamposGrid[$i][4])=='Bueno'){$CampoEstado2=utf8_decode(""); $CampoEstado1 = utf8_decode('x');
	// }elseif(utf8_decode(@$ExplodeCamposGrid[$i][4])=='Malo'){$CampoEstado1=utf8_decode(""); $CampoEstado2 = utf8_decode('x'); }
	// else{ @$CampoEstado1 = utf8_decode(' '); @$CampoEstado2 = utf8_decode(' '); }
	
	$numero=$i+1;
	$pdf->SetLineWidth(0);
	$pdf->SetXY(23.1, $y);
	$pdf->Cell(11.7,5.5,"$numero",1,1,'C');  //N°
	$pdf->SetXY(34.8, $y);
	if(strlen($ubicacion)>31){ $pdf->SetFontSize(6); }else{ $pdf->SetFontSize(7); }
	$pdf->Cell(32.8,5.5,"$ubicacion",1,1,'L');  //UBICACION 1	
	$pdf->SetXY(67.70,  $y);
	$pdf->Cell(10.8,5.5,"$CampoConsumo1",1,0,'C');  //%consumo1
	$pdf->SetXY(78.5,  $y);
	$pdf->Cell(11.7,5.5,"$CampoConsumo2",1,0,'C');  //%consumo2
	$pdf->SetXY(90.2,  $y);	
	$pdf->Cell(11.65,5.5,"$CampoConsumo3",1,0,'C');  //%consumo3
	$pdf->SetXY(101.8,  $y);
	$pdf->Cell(11.70,5.5,"$CampoConsumo4",1,0,'C');  //%consumo4
	$pdf->SetXY(113.43,  $y);
	$pdf->Cell(11.70,5.5,"$CampoConsumo5",1,0,'C');  //%consumo5
	$pdf->SetXY(125.17,  $y);
	$pdf->Cell(16.1,5.5,"$limpieza",1,0,'C');  //%limpieza
	$pdf->SetXY(141.3,  $y);
	$pdf->Cell(14,5.5,"$CampoEstado1",1,0,'C');  //bueno
	$pdf->SetXY(155.3,  $y);
	$pdf->Cell(11.45,5.5,"$CampoEstado2",1,0,'C');  //malo
	$pdf->SetXY(166.7, $y);
	$pdf->Cell(30.2,5.5,"$observacion",1,0,'L');  //observaciones
 ...
 elseif($count > 40 and $count < 49){  $pdf->Image('PhpExtras/RLPdfFooter.png', 23.5,192.5,'','','png',''); @$yResponsables = 225; }
 ...
 elseif($count > 74 and $count < 83){  $pdf->Image('PhpExtras/RLPdfFooter.png', 23.5,192.5,'','','png',''); @$yResponsables = 225; }
 ...
 elseif($count > 108 and $count < 117){  $pdf->Image('PhpExtras/RLPdfFooter.png', 23.5,192.5,'','','png',''); @$yResponsables = 225; }
 ...
 elseif($count > 142 and $count < 151){  $pdf->Image('PhpExtras/RLPdfFooter.png', 23.5,192.5,'','','png',''); @$yResponsables = 225; }
 ...
 elseif($count > 176 and $count < 185){  $pdf->Image('PhpExtras/RLPdfFooter.png', 23.5,192.5,'','','png',''); @$yResponsables = 225; }
...
$count = strlen($Obser);
if($count >= 110) {
	$obser2 = substr($Obser, 0, 110); 
	$restoObser=substr($Obser, 110);
	$pdf->SetXY(25, $yObser);
...
$nombre_fichero = '../../web/UploadPdf/PdfFumigacion/ReportRodenticidas/'.$sucursal.'';

if (file_exists($nombre_fichero)) {
	$pdf->Output('F',"../../web/UploadPdf/PdfFumigacion/ReportRodenticidas/$sucursal/$noFolio-$noCliente.pdf",'UTF-8 (true)');
}else {
