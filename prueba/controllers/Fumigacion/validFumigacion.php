...
$ListPendientes=$Fumigacion->listPendientesValid(@$_COOKIE['id_emp']);
$ListPendientesReportFumi=$Fumigacion->listPendientesValidReportFumi(@$_COOKIE['id_emp']);

if(isset($_POST['formUpdateCertifiFumiTec'])){
		$Fumigacion->updateCertiFumiTec($_POST);
	}else{
 ?>	
			<!DOCTYPE html>
			<html lang="en">
				...
				$breadcrumbPrincipal='Certificados';
				$variables= array('sufijo'=>$sufijo,'sufijoMenu'=>$sufijoMenu,'breadcrumb'=>$breadcrumb,'breadcrumbPrincipal'=>$breadcrumbPrincipal,'loadCssHeader'=>$loadCssHeader,'ListPendientes'=>$ListPendientes,'ListPendientesReportFumi'=>$ListPendientesReportFumi);

				/*nombre del archivo a llamar y manda las variables*/
				view('Fumigacion/validFumigacion',$variables);  ?>		
