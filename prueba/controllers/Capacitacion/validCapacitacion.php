...
$listPendientesValidPagoAlum=$Capacitacion->listPendientesValidPagoAlum(@$_COOKIE['id_emp']); 
$listPendientesValidConstanPC=$Capacitacion->listPendientesValidConstanPC(@$_COOKIE['id_emp']); 
$listPendientesValidInstruct=$Capacitacion->listPendientesValidInstruct(@$_COOKIE['id_emp']); 

if($_POST['action']=='ValidPagoCurso'){
		$Capacitacion->updateValidPagoCurso($_POST);
	}else if($_REQUEST['action']=='SearchDatosCotizacion'){
			$Capacitacion->SearchDatosCotizacion($_REQUEST); exit; 
	}else if($_REQUEST['action']=='SearchDatosForCostear'){
			$Capacitacion->SearchDatosForCostear($_REQUEST); exit; 
	}else if($_REQUEST['action']=='GenerateFormatCotiza'){
			$Capacitacion->GenerateFormatCotiza($_REQUEST); exit; 
	}else if($_POST['action']=='ValidCotizacionCurso'){
		$Capacitacion->ValidCotizacionCurso($_POST);
	}else if($_REQUEST['action']=='ShowDocumentInstruc'){
			$Capacitacion->showDocumentInstruc($_REQUEST); exit; 
	}else if($_REQUEST['action']=='UpdaValidDoc'){
			$Capacitacion->UpdaValidDoc($_REQUEST); exit; 
	}else{
 ?>	
			<!DOCTYPE html>
			<html lang="en">
				...
				$breadcrumbPrincipal='Nuevas Inscripciones(Cursos)';
				$variables= array('sufijo'=>$sufijo,'sufijoMenu'=>$sufijoMenu,'breadcrumb'=>$breadcrumb,'breadcrumbPrincipal'=>$breadcrumbPrincipal,'loadCssHeader'=>$loadCssHeader,'listPendientesValidPagoAlum'=>$listPendientesValidPagoAlum,'listPendientesValidConstanPC'=>$listPendientesValidConstanPC,'listPendientesValidInstruct'=>$listPendientesValidInstruct);

				/*nombre del archivo a llamar y manda las variables*/
				view('Capacitacion/validCapacitacion',$variables);  ?>		
					...
					<script src="//cdnjs.cloudflare.com/ajax/libs/x-editable/1.5.0/bootstrap3-editable/js/bootstrap-editable.min.js"></script>
					 ...
					 <script>
						$(document).ready(function(){
							App.init();						
							$('.footable, .tableConstan').footable();
							$('.txtPrecioUnit, .txtImporte, .txtSubtotal, .txtIva, .txtTotal').mask("#,##0.00", {reverse: true});
						});
					</script>
