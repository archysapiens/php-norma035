<?php
session_start();
if(isset($_SESSION['sesionUsuario']) & @$_COOKIE['id']!='' & $_REQUEST['SetIdClient']!='' & $_REQUEST['SetPdfPHChecList']!=''){
$SetIdClient = $_REQUEST['SetIdClient'];
$SetPdfChecList = $_REQUEST['SetPdfPHChecList'];
$sufijo='../../'; $sufijoMenu='../';
...
if(isset($_POST['formCreatedNewCheckL'])){
		$Extintores->saveCheckList($_POST);
	}else if(isset($_POST['GererateJsonCL'])){
		$Extintores->gerateJsonCL($_POST);}	
	else{
 ?>
			<!DOCTYPE html>
			<html lang="en">
				...
				<body> <?php
					...
					$breadcrumbPrincipal='Check List Extintores de Salida';
					$variables= array('sufijo'=>$sufijo,'sufijoMenu'=>$sufijoMenu,'breadcrumb'=>$breadcrumb,'breadcrumbPrincipal'=>$breadcrumbPrincipal,'loadCssHeader'=>$loadCssHeader,'SetIdClient'=>$SetIdClient,'SetPdfChecList'=>$SetPdfChecList);

					view('Extintores/checkList',$variables); 
					include("../login/control_session.php"); ?>
						...
						window.addEventListener("load", function() {
						  frm_CheckList.txtnoCliente.addEventListener("keypress", soloNumeros, false);
						});
